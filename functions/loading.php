<?php
	function loadingMoreStories($category){
		if(isset($_POST['post_id'])){
			$post_id = $_POST['post_id'];
			include '../includes/connect.inc.php';
			$query = $handler->query("SELECT * FROM `pn_post` WHERE `post_status` !='Trash' && `id` < '$post_id' && `post_category`='$category' ORDER BY `id` DESC LIMIT 8");
				while($query_run = $query->fetch()){
					...
					$last_id = $handler->query("SELECT `id` FROM `pn_post` WHERE `post_status` !='Trash' && `post_category`='$category' ORDER BY `id` ASC LIMIT 1");
							$id_no = $last_id->fetch();
								$last_id_no = $id_no['id'];
								$post_id;	
